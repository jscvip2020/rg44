            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card m-auto" style="max-width: 600px;">
                        <div class="card-body">
                            <form class="form-horizontal" method="POST" action="{{ route('medias.store') }}">
                                {{ csrf_field() }}
                                ...
                                <div class="form-group">
                                    <label for="{{$input}}" class="col-md-12 control-label">Icone da média social</label>

                                    <div class="col-md-12">
                                        <input id="{{$input}}" type="text" class="col-md-10 form-control {{ $errors->has($input) ? ' is-invalid' : '' }}" name="{{$input}}"
                                               placeholder="fa-youtube"
                                               value="{{ isset($row) ? $row->$input : old($input) }}">
                                        <i id="icon" class="fab text-primary {{ isset($row) ? $row->$input : old($input) }} fa-2x float-right"></i>
                                        <small id="apikeyHelper" class="form-text text-muted">
                                            Um icone do <a href="https://fontawesome.com/v4.7.0/icons/">Font Awesome</a> Ex: fa-youtube
                                        </small>
                                        @if ($errors->has($input))
                                            <span class="help-block invalid-feedback"><strong>{{ $errors->first($input) }}</strong></span>
                                        @endif
