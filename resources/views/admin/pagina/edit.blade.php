 ...
 * Time: 13:19
 */ ?>
@section('titulo', ' - Editando Pagina')
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card m-auto" style="max-width: 600px;">
                        <div class="card-body">
                            <form class="form-horizontal" method="post" action="{{ route('paginas.update', $pagina->id) }}">
                                @method('put')
                                {{ csrf_field() }}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        <input id="{{$input}}" type="text" class="form-control {{ $errors->has($input) ? ' is-invalid' : '' }}" name="{{$input}}"
                                               placeholder="Insira um título para a Pagina"
                                               value="{{ isset($pagina) ? $pagina->$input : old($input) }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        <textarea id="{{$input}}" type="textarea"
                                                  ...
                                                  name="{{$input}}">{{ isset($pagina) ? $pagina->$input : old($input) }}</textarea>
                                        @if ($errors->has($input))
                                            <span class="help-block invalid-feedback"><strong>{{ $errors->first($input) }}</strong></span>
                                        @endif
                                    </div>
